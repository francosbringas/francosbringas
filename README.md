...
**Back-End**
`PHP 8` `PDO` `Laravel 11` `Eloquent ORM` `Python` `C++` `REST APIs`
 
**Database**
`MySQL` `SQLite` `Relational Design` `Normalization` `SQL`
 
**Hardware & IoT**
`Raspberry Pi 3` `GPIO` `Sensors` `Servomotors` `SSH Automation`
 
**Networking**
`Cisco (routers & switches)` `VLSM Subnetting` `IPv4/IPv6` `TCP/IP` `Linux`
 
**Tools**
`Git` `GitHub` `VS Code` `XAMPP` `VirtualBox` `AnyDesk`
 
**AI Workflow**
`Claude` `Copilot` `Gemini` `Prompt Engineering`
...
### 🎓 [School Management System](https://github.com/francosbringas/sistema-gestion-escolar)
Multi-role academic web platform for students, teachers, homeroom teachers and guardians. bcrypt authentication, expiring recovery tokens, role-based access control enforced on every endpoint.
 
> `PHP` `PDO` `MySQL` `Bootstrap 5` `jQuery` `JavaScript`
 
### 🏙️ [MunichTech EXPO 2026](https://github.com/francosbringas/munichtech-expo)
Venture capital and deep-tech platform built in 24 hours as a technical evaluation task. Full-stack Laravel 11 application with Google OAuth 2.0, collaboration request workflows, project management with milestone and task hierarchies, role-based access control, admin moderation dashboard and AI-style rule-based matchmaking for investor–startup alignment. Deployed on Render.
 
> `Laravel 11` `PHP 8.2` `Blade` `Bootstrap 5` `SQLite` `Eloquent ORM` `Google OAuth` `Laravel Socialite`
